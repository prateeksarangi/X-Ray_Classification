...
class User(db.Model):
    ...
    def __repr__(self):
        return f"User('{self.pname}','{self.gender}','{self.age}','{self.pno}','{self.result}')"
...
def upload_file():
    if request.method == 'POST':
        ...
        if output[0][0] == 1:
            ...
            labelans = "Pneumonia"
        user1 = User(pname=request.form['name'], gender=request.form['gender'], age=request.form['age'], pno=request.form['pno'], result=labelans)
        db.session.add(user1)
        db.session.commit()
        doctor1 = Doctor(dname=request.form['d_name'], did=request.form['d_id'], dno=request.form['d_mob'])
        db.session.add(doctor1)
        db.session.commit()
        return render_template('index.html',predict=labelans,image_file=image_file,pname=user1.pname,gender=user1.gender,age=user1.age,pno=user1.pno,dname=doctor1.dname,dno=doctor1.dno,did=doctor1.did)
